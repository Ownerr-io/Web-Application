    ...
    <Link href={`/startup/${startup.slug}`}>
      <div className="startup-card p-5 h-full flex flex-col group cursor-pointer block">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div
            className="w-12 h-12 rounded-[8px] flex items-center justify-center font-bold text-xl border border-black/5 shrink-0"
            style={{ backgroundColor: startup.logoColor, color: 'rgba(0,0,0,0.7)' }}
          >
            {startup.name.charAt(0)}
          </div>
          <div className="flex-1 min-w-0 pr-8">
            <h3 className="font-bold text-base truncate group-hover:underline">{startup.name}</h3>
        ...
        <div className="grid grid-cols-3 gap-2 mt-auto pt-3 border-t border-border">
          <div>
            <div className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider mb-1">Revenue</div>
