...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!base) {
    res.status(503).json({
      error:
        "Optional verification routes are not configured. Marketplace uses Supabase only; set verification invoke URLs in platform_internal_config or SYNC_WORKER_INTERNAL_URL if you enable async verification.",
    });
    return;
  }
