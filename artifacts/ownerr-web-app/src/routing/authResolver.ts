...
export function sanitizePostAuthRedirectParam(
  raw: string | null | undefined,
): string | null {
  ...
  if (isGuestMarketingPath(safe)) {
    ...
    if (product === "ownerr-network")
      return productDashboardPath("ownerr_network");
    if (safe.startsWith("/marketplace/app"))
      return productDashboardPath("marketplace");
...
export function resolveAuthenticatedAppEntry(input: {
  ...
  if (
    ...
    if (safeReturn && isPathAllowedForDemoMarketplaceLock(safeReturn))
      return safeReturn;
