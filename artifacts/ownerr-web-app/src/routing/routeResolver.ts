...
export function isAuthRoute(pathname: string): boolean {
  ...
  return path.startsWith('/products/') && /\/(login|register|callback|forgot-password)$/.test(path);
}
...
export function isGuestMarketingPath(pathname: string): boolean {
  ...
  return route.layoutShell === 'marketing' || route.layoutShell === 'marketplace-public';
}
