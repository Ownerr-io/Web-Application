...
export const PRODUCT_ITEMS: ProductNavItem[] = [
  {
    id: 'marketplace',
    label: 'Marketplace',
    description: 'Acquire startups, manage deals, and run buyer or founder desk workflows.',
  ...
  },
  {
    id: 'ownerr-os',
    label: 'OWNERR OS',
    description: 'Founder viral loop, share cards, and referral growth for startups.',
  ...
  },
  {
    ...
    description: 'Profile-first discovery, reputation, referrals, and platform credits.',
...
const productAppMenuLinks: ProductsMenuLink[] = PRODUCT_ITEMS.map((p) => ({
  id: p.id,
  label: p.label,
...
export function getProductsDropdownSections(): ProductsDropdownSection[] {
  return [
    { id: 'hub', title: null, items: [PRODUCTS_HUB_LINK] },
    { id: 'apps', title: 'Product apps', items: productAppMenuLinks },
  ];
}

export function getAllProductsMenuHrefs(): string[] {
  return getProductsDropdownSections().flatMap((s) => s.items.map((i) => i.href));
}

export const PUBLIC_NAV_CTA_ITEMS: PublicNavCta[] = [
  { id: 'get-started', label: 'Get Started', href: AUTH_ROUTES.start, variant: 'primary' },
];
...
export function marketplaceWorkspaceForRole(role: AuthRole): string {
  return role === 'buyer' ? MARKETPLACE_ROUTES.buyerDashboard : MARKETPLACE_ROUTES.sellerDashboard;
}
...
export function isNavLinkActive(location: string, href: string): boolean {
  ...
  if (base === MARKETPLACE_ROUTES.root) {
    return location === MARKETPLACE_ROUTES.root || location === `${MARKETPLACE_ROUTES.root}/`;
  }
...
export function isProductsDropdownActive(location: string): boolean {
  return getAllProductsMenuHrefs().some((href) => isNavLinkActive(location, href));
}

const PRODUCT_SHELL_LABELS: readonly { id: AuthenticatedWorkspace; label: string }[] = [
...
const MARKETPLACE_BUYER_SIDEBAR: readonly SidebarNavSection[] = [
  {
    title: 'Buyer',
    items: [
      marketplaceNavItem('marketplace.buyer.overview', 'Overview', MARKETPLACE_ROUTES.buyerDashboard, LayoutDashboard, ['buyer']),
      marketplaceNavItem('marketplace.buyer.browse', 'Browse Startups', MARKETPLACE_ROUTES.buyerBrowse, Search, ['buyer']),
      marketplaceNavItem('marketplace.buyer.interests', 'My Interests', MARKETPLACE_ROUTES.buyerInterests, Star, ['buyer']),
      marketplaceNavItem('marketplace.buyer.bids', 'My Bids', MARKETPLACE_ROUTES.buyerBids, Activity, ['buyer']),
    ],
  },
];
...
const MARKETPLACE_SELLER_SIDEBAR: readonly SidebarNavSection[] = [
  {
    title: 'Seller',
    items: [
      marketplaceNavItem('marketplace.seller.overview', 'Overview', MARKETPLACE_ROUTES.sellerDashboard, LayoutDashboard, ['founder']),
      marketplaceNavItem('marketplace.seller.listings', 'My Listings', MARKETPLACE_ROUTES.sellerListings, Briefcase, ['founder']),
      marketplaceNavItem('marketplace.seller.inbox', 'Inbox', MARKETPLACE_ROUTES.sellerInbox, MessageSquare, ['founder']),
      marketplaceNavItem('marketplace.seller.verification', 'Verification', MARKETPLACE_ROUTES.sellerVerification, Shield, ['founder']),
    ],
  },
];

function isSidebarItemVisible(item: SidebarNavItemDef, subject: AccessSubject): boolean {
  if (!canAccessPath(item.href, subject)) return false;
  if (!item.requiredRoles?.length) return true;
  if (item.requiredRoles.includes('network_member')) return subject.networkProfile;
  if (!subject.deskUser) return false;
  return item.requiredRoles.includes(subject.deskUser.role);
}

export function getMarketplaceSidebarSections(subject: AccessSubject): SidebarNavSection[] {
  const role = subject.deskUser?.role;
  const sections =
    role === 'founder' ? MARKETPLACE_SELLER_SIDEBAR : role === 'buyer' ? MARKETPLACE_BUYER_SIDEBAR : [...MARKETPLACE_BUYER_SIDEBAR, ...MARKETPLACE_SELLER_SIDEBAR];
  return sections
    .map((section) => ({
      title: section.title,
...
const OWNERR_OS_SIDEBAR: readonly SidebarNavSection[] = [
  {
    title: 'Founder',
    items: [
      ownerrNavItem('ownerr-os.dashboard', 'Overview', PRODUCT_ROUTES.ownerrOsDashboard, LayoutDashboard),
      ownerrNavItem('ownerr-os.listings', 'My Startups', PRODUCT_ROUTES.ownerrOsListings, Briefcase),
      ownerrNavItem('ownerr-os.referrals', 'Invite & Stats', PRODUCT_ROUTES.ownerrOsReferrals, UserPlus),
      ownerrNavItem('ownerr-os.analytics', 'Analytics', PRODUCT_ROUTES.ownerrOsAnalytics, BarChart3),
    ],
  },
  {
    title: 'Account',
    items: [ownerrNavItem('ownerr-os.settings', 'Settings', PRODUCT_ROUTES.ownerrOsSettings, Settings)],
  },
];

export function getOwnerrOsSidebarSections(subject: AccessSubject): SidebarNavSection[] {
...
const SIDEBAR_NAV_REGISTRY: readonly SidebarNavItemDef[] = [
  ...
  {
    id: 'ownerr-os.referrals',
    group: 'ownerr-os',
    label: 'Invite & Stats',
    href: PRODUCT_ROUTES.ownerrOsReferrals,
    icon: UserPlus,
    requiredRoles: ['founder'],
  },
  {
  ...
  {
    id: 'ownerr-os.settings',
    group: 'ownerr-os',
    label: 'Settings',
    href: PRODUCT_ROUTES.ownerrOsSettings,
    icon: Settings,
    requiredRoles: ['founder'],
  },
  {
  ...
  {
    id: 'ownerr-network.settings',
    group: 'ownerr-network',
    label: 'Settings',
    href: PRODUCT_ROUTES.ownerrNetworkSettings,
    icon: Settings,
    requiredRoles: ['network_member'],
  },
] as const;
...
export function getAuthenticatedSidebarNav(
  ...
  if (group === 'marketplace') {
    return getMarketplaceSidebarSections(subject).flatMap((section) => section.items);
  }
  if (group === 'ownerr-os') {
    return getOwnerrOsSidebarSections(subject).flatMap((section) => section.items);
  }
  return SIDEBAR_NAV_BY_GROUP[group].filter((item) => isSidebarItemVisible(item, subject));
}

export function getAuthenticatedWorkspaceLabel(workspace: AuthenticatedWorkspace): string {
  return PRODUCT_SHELL_LABELS.find((p) => p.id === workspace)?.label ?? 'Product';
}
