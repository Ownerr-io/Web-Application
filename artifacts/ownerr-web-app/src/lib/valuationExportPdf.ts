...
function logoDisplaySize(naturalW: number, naturalH: number, maxHeight: number): { w: number; h: number } {
...
async function loadOwnerrLogoAsset(bg: 'white' | 'dark'): Promise<LogoAsset | null> {
  try {
...
function paintRunningFooter(doc: jsPDF, pageNum: number, pageCount: number, reportRef: string): void {
  ...
  const footerLeft = doc.splitTextToSize(`${BRAND_NAME} · Confidential · ${reportRef}`, contentWidth(doc) * 0.72);
...
function ensureSpace(doc: jsPDF, y: number, needed: number, reportRef: string): number {
  const { h } = pageSize(doc);
...
function buildConsultantView(bundle: ValuationExportBundle, model: ValuationExportDocument) {
  const { inputs, outputs } = bundle;
  const arr = inputs.arr > 0 ? inputs.arr : Math.max(0, inputs.mrr) * 12;
  const ltvCac =
    inputs.cac > 0 && inputs.ltv > 0 ? (inputs.ltv / inputs.cac).toFixed(1) : null;
  ...
  const timingParagraph = `The timing corridor is characterized as "${outputs.marketTiming}," with capital efficiency at ${
    inputs.burnMultiple > 0 ? `${inputs.burnMultiple.toFixed(1)}× burn` : 'an unmeasured burn multiple'
  ...
  const capitalEff =
    model.marketTiming.find((r) => r.label === 'Capital efficiency anchor')?.value ?? '—';
...
function paintMemorandumHeader(
  ...
  let logoY = 0;
  if (logo) {
    const size = logoDisplaySize(logo.widthPx, logo.heightPx, LOGO_MAX_HEIGHT_PT);
    logoW = size.w;
    logoH = size.h;
  ...
  y = writeLines(doc, y, titleLines, MARGIN_X, model.reportRef, 17, leadingPt(17), 'bold');

  const companyLines = doc.splitTextToSize(model.companyName, cw);
  y = writeLines(doc, y, companyLines, MARGIN_X, model.reportRef, 12, leadingPt(12), 'bold');

  doc.setTextColor(...COLORS.muted);
  const subLines = doc.splitTextToSize('Prepared for diligence · Final synthesis', cw);
  y = writeLines(doc, y, subLines, MARGIN_X, model.reportRef, 8.5, leadingPt(8.5), 'normal');

  return y + LAYOUT.afterTitleBlock;
}

export async function downloadValuationPdfImpl(bundle: ValuationExportBundle): Promise<void> {
  ...
  // ── EXECUTIVE SUMMARY ──────────────────────────────────────────────────────
  y = writeSectionTitle(doc, y, 'Executive summary', model.reportRef, { first: true });
  y = writeBody(doc, y, model.executiveSummary, model.reportRef);
  y = addCompactTable(doc, y, ['Metric', 'Reading'], view.snapshotRows, model.reportRef);
  ...
  y = writeSectionTitle(doc, y, 'Priority intelligence themes', model.reportRef);
