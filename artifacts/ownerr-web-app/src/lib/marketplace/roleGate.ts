...
import { provisionBuyerForUser, provisionSellerForUser } from '@/lib/marketplace/service';
import type { User } from '@supabase/supabase-js';

export type MarketplaceAction = 'bid' | 'interest' | 'buy' | 'offer' | 'list_startup' | 'verify' | 'accept_offer';

export type RoleGateResult =
  | { ok: true }
  | { ok: false; redirect: string; reason: 'auth' | 'wrong_role' | 'switch_workspace' };
...
export async function gateMarketplaceAction(input: {
  ...
  const sellerActions: MarketplaceAction[] = ['list_startup', 'verify', 'accept_offer'];

  if (!input.session || !input.authUser) {
    const role = buyerActions.includes(input.action) ? 'buyer' : 'seller';
    return { ok: false, redirect: marketplaceAuthStartUrl({ role, returnTo: input.returnTo }), reason: 'auth' };
  }
...
export function resolveMarketplaceAppEntry(input: {
  ...
  if (input.hasBuyer && !input.hasSeller) return MARKETPLACE_ROUTES.buyerDashboard;
  if (input.hasSeller && !input.hasBuyer) return MARKETPLACE_ROUTES.sellerDashboard;
  if (input.metadataRole === 'buyer') return MARKETPLACE_ROUTES.buyerDashboard;
  if (input.metadataRole === 'founder') return MARKETPLACE_ROUTES.sellerDashboard;
