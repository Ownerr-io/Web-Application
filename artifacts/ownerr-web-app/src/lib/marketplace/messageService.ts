...
async function sellerProfileIdForStartup(startup: {
  ...
  if (!sellerId) throw new MarketplaceError('Seller profile required', 'profile_required');
  return sellerId;
}
...
export async function sendMessage(input: {
  ...
  if (!trimmed) throw new MarketplaceError('Message cannot be empty', 'validation');
  const { data, error } = await requireSupabase()
    .from('messages')
    .insert({
...
type ConvRow = {
  ...
  updated_at?: string;
  created_at: string;
  startups: { slug: string; title: string } | { slug: string; title: string }[] | null;
...
async function enrichThread(row: ConvRow, authUserId: string): Promise<InboxThread> {
...
export async function listInboxForUser(authUserId: string): Promise<InboxThread[]> {
  ...
  async function load(filterCol: 'buyer_profile_id' | 'seller_profile_id', profileId: string) {
    const { data, error } = await requireSupabase()
      .from('conversations')
      .select(selectCols)
  ...
  return Promise.all([...byId.values()].map((r) => enrichThread(r, authUserId)));
}
...
export async function appendMarketplaceThreadMessage(input: {
  ...
  return {
    ...
    stage: (interest?.status as MarketplaceInterestRecord['stage']) ?? 'interested',
    messages: [
      {
