...
export function mapStartupRow(row: StartupRow): Startup {
  ...
  const mrr =
    typeof row.annual_revenue === 'number' && row.annual_revenue > 0
      ...
      : 0;
  const asking = row.asking_price != null ? Number(row.asking_price) : undefined;
  const base: Startup = {
    slug: row.slug,
    name: row.title,
    ...
    price: asking,
    multiple:
      asking && mrr > 0 ? Math.round((asking / (mrr * 12)) * 10) / 10 : undefined,
    forSale: row.status === 'published' && asking != null,
    founderHandle: row.founder_handle ?? 'unknown',
    founderDisplayName: readMeta<string | undefined>(meta, 'founder_display_name', undefined),
  ...
  return {
    ...
    acquisitionPower: readMeta(meta, 'acquisition_power', scored.acquisitionPower),
  };
}
