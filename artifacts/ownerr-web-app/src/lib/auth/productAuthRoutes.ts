...
export type ProductAuthPage = 'login' | 'register' | 'callback' | 'forgot-password';
...
export function resolveProductAuthPath(slug: AppSlug, page: ProductAuthPage): string {
  if (slug === 'marketplace') {
...
export function parseProductSlugFromAuthPath(pathname: string): AppSlug | null {
  ...
  if (path.startsWith('/marketplace/login') || path.startsWith('/marketplace/register')) {
    return 'marketplace';
  }
  if (path.startsWith('/marketplace/callback') || path.startsWith('/marketplace/forgot-password')) {
    return 'marketplace';
  }
  return null;
}
...
export function isProductAuthPath(pathname: string): boolean {
  const path = pathname.replace(/\/$/, '');
  if (path.startsWith('/marketplace/') && /\/(login|register|callback|forgot-password)$/.test(path)) {
    return true;
  }
  return parseProductSlugFromAuthPath(pathname) !== null && /\/(login|register|callback|forgot-password)$/.test(pathname);
}
