...
export function isAppSlug(value: string | null | undefined): value is AppSlug {
  return value === 'ownerr_os' || value === 'marketplace' || value === 'ownerr_network';
}
...
export function captureProductIntentFromPath(pathname: string): AppSlug | null {
  ...
  if (path === PRODUCT_ROUTES.ownerrOsLanding || path.startsWith('/products/ownerr-os')) {
    return 'ownerr_os';
  }
  if (path === PRODUCT_ROUTES.ownerrNetworkLanding || path.startsWith('/products/ownerr-network')) {
...
export function appSlugToAuthProduct(slug: AppSlug): 'ownerr-os' | 'ownerr-network' | null {
