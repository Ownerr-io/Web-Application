...
} from "@/components/valuation/valuationQuestions";
...
export function buildInvestorNarrative(
  inputs: ValuationInputs,
  o: ValuationOutputs,
): string[] {
  const lines: string[] = [];
...
export function buildValuationExportDocument(
  bundle: ValuationExportBundle,
): ValuationExportDocument {
  ...
  const companyName =
    startupName || meta.startupName.trim() || "Subject company";
  ...
  const modelEngineInputs: ReportRow[] = [
    ...
    {
      label: "Runway",
      value: `${inputs.runwayMonths} months`,
      context: "Model parameter",
    },
    {
      label: "Founder goal",
      value: founderGoalLabels[inputs.founderGoal],
      context: "Model parameter",
    },
  ];
