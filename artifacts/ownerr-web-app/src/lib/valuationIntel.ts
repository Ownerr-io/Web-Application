...
export function computeValuationIntel(i: ValuationInputs): ValuationOutputs {
  ...
  const burnAdj =
    i.burnMultiple > 0
      ? clamp(1.35 - (i.burnMultiple - 1) * 0.08, 0.75, 1.2)
      : 1;
  ...
  const growthQuality = clamp(
    42 +
      i.monthlyGrowthPct * 1.8 -
      i.churnPctMonthly * 2.2 +
      (i.profitMarginPct > 0 ? 8 : -4),
    12,
    96,
  );
  ...
  if (investorInterest > 78 && acquisitionHeat > 72)
    marketTiming = "Favorable exit window";
  ...
  if (i.founderGoal === "exit" && acquisitionHeat > investorInterest - 5) {
    ...
    strategicRecommendation =
      "Compounding economics favor duration over near-term liquidity.";
...
export function buildStrategicInsights(
  ...
  add({
    ...
    confidence: clamp(
      55 + Math.abs(o.acquisitionHeat - o.investorInterest) * 0.25,
      50,
      86,
    ),
  ...
  if (i.runwayMonths >= 8) {
    add({
      ...
      confidence: clamp(
        58 + (i.runwayMonths >= 18 ? 8 : 0) - (i.burnMultiple > 2.2 ? 10 : 0),
        44,
        88,
      ),
