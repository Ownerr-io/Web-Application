...
export function StatsDashboard() {
  ...
  const rows = useMemo(
    () => buildStatsDataset(publicStartups),
    [publicStartups],
  );
  ...
  const followerDist = useMemo(
    () => distributionBuckets(rows, "xFollowers"),
    [rows],
  );
  ...
  const dataGrowthVsGrowth = useMemo(
    () =>
      rows.map((r) => ({
        ...
        founderHandle: r.founderHandle,
        founderAvatarSeed:
          getFounderByHandle(r.founderHandle, founders)?.avatarSeed ??
          r.founderHandle,
      })),
    [rows, founders],
  );
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-10 pb-16 font-sans text-[color:var(--terminal-fg)]">
      <div>
        <h1 className="font-mono text-2xl font-bold text-[color:var(--terminal-display)]">
          Stats
        </h1>
        <p className="mp-body mt-1 text-sm">
          Verified revenue, founders, and markets across Ownerr
        </p>
      </div>
          ...
          <ResponsiveContainer>
            ...
            >
              ...
              <YAxis
                ...
                allowDecimals={false}
                width={40}
                label={yAxisTitle("Startups", chart.tick.fill)}
              />
              <Tooltip
                contentStyle={{
      ...
      <section className={card}>
        ...
        <p className={sub}>
          Average months to reach revenue milestones (modelled from verified MRR
          + age)
        </p>
        <div className="mt-4 h-72 w-full min-w-0 max-w-full">
          <ResponsiveContainer>
          ...
          <ResponsiveContainer>
            ...
            >
              ...
              <YAxis
                ...
                allowDecimals={false}
                width={40}
                label={yAxisTitle("Founders", chart.tick.fill)}
              />
              <Tooltip
                contentStyle={{
                  background: chart.tooltipBg,
      ...
      <section className={card}>
        <div className="mb-2 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className={sub}>
              ARR (y) vs founder 𝕏 reach (x). Dashed guides at $1.5M and ~300
              followers.
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
              <span>Log 𝕏</span>
              <Switch
                checked={logFollowers}
                onCheckedChange={setLogFollowers}
              />
            </div>
            <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
      ...
      <section className={card}>
        <div className="mb-2 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className={sub}>
              Above the diagonal = revenue growth outpaced followers · Below =
              followers grew faster
            </p>
        ...
        <div className="mt-2 rounded-lg border border-border bg-muted/30 px-3 py-2 text-center text-xs text-muted-foreground">
          Revenue outpaced followers in {growthCompare.pct}% of cases (n=
          {growthCompare.n})
        </div>
      </section>
          ...
          <ResponsiveContainer>
            <ScatterChart margin={M_SCATTER}>
              ...
              <XAxis
                ...
                label={xAxisTitle(
                  "Time in business (days, log scale)",
                  chart.tick.fill,
                )}
              />
              <YAxis
              ...
              {olympics.map((r, i) => (
                <tr key={r.iso} className="hover:bg-muted/50">
                  ...
                  <td className={cn(td, "font-mono font-bold")}>
                    {r.name}{" "}
                    <span className="text-muted-foreground">({r.iso})</span>
                  </td>
      ...
      <section className={card}>
        ...
        <p className={sub}>
          Aggregated MRR by inferred primary stack (deterministic from category
          + slug)
        </p>
              ...
              {highLow.map((r, i) => {
                ...
                return (
                  <tr key={r.slug} className="hover:bg-muted/50">
                    ...
                    <td className={td}>
                      <Link
                        href={marketplacePath(`/startup/${r.slug}`)}
      ...
      <div className="flex items-center gap-2 border-b border-border pb-2">
        ...
        <div>
          ...
          <div className="text-[10px] text-muted-foreground">
            ARR {formatCurrency(p.y)}
          </div>
        </div>
      </div>
      <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
        <img
          src={founderAvatarUrl(p.founderAvatarSeed)}
          alt=""
...
function TipGrowth({ active, payload }: any) {
  ...
  return (
    <div className="z-50 rounded-lg border border-border bg-popover p-2 text-xs text-popover-foreground shadow-xl">
      ...
      <div className="text-muted-foreground">
        Revenue +{p.y.toFixed(1)}% · Followers +{p.x.toFixed(1)}%
      </div>
    </div>
  );
}
...
function TipTime({ active, payload }: any) {
  ...
  return (
    <div className="z-50 rounded-lg border border-border bg-popover p-2 text-xs text-popover-foreground shadow-xl">
      ...
      <div className="text-muted-foreground">
        ~{Math.round(p.x)} days in business
      </div>
    </div>
  );
}
