...
import { persistProductIntent, productDashboardPath } from '@/lib/auth/productLock';
...
import { ensureBuyerProfile, ensureSellerProfile } from '@/lib/marketplace/profiles';
...
export function GetStartedFlow() {
  ...
  const enterMarketplace = useCallback(
    async (role: AuthRole) => {
      ...
      try {
        ...
        setError(e instanceof Error ? e.message : 'Could not open marketplace desk');
      } finally {
        setBusy(false);
      }
    },
  ...
  const enterOwnerrNetwork = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Could not open Ownerr Network');
    } finally {
      setBusy(false);
    }
  }, [authUser?.id, navigate, setActiveProduct]);
  ...
  useEffect(() => {
    ...
    if (productParam === 'ownerr-network' || pendingProduct === 'ownerr_network') {
      void enterOwnerrNetwork();
    }
  }, [loading, session, productParam, enterMarketplace, enterOwnerrOs, enterOwnerrNetwork]);
    ...
    return (
      <MarketingLayout hideProductContext>
        <div className="saas-section-shell py-16 md:py-24">
          ...
          <h1 className="marketing-hero-title mt-2">How are you using the desk?</h1>
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2">
            ...
            >
              ...
              <p className="marketing-body-sm mt-2">Browse listings, place bids, and track interests.</p>
            </button>
            <button
              type="button"
              disabled={busy}
            ...
            >
              ...
              <p className="marketing-body-sm mt-2">Manage listings, inbox, and verification.</p>
            </button>
          </div>
        </div>
      </MarketingLayout>
  ...
  return (
    <MarketingLayout hideProductContext>
      <div className="saas-section-shell py-16 md:py-24">
        ...
        <p className="marketing-lead mt-3 max-w-xl">
          Pick where you want to work. Marketplace buyers and founders get separate desks; other products open
          in their own app.
        </p>
        {error ? <p className="mt-4 text-sm font-semibold text-destructive">{error}</p> : null}
        <ul className="mt-10 grid gap-4 md:grid-cols-3">
          {PRODUCT_ITEMS.map((product) => (
            <li key={product.id}>
              ...
              >
                <h2 className="text-lg font-bold text-[color:var(--terminal-fg)]">{product.label}</h2>
                <p className="marketing-body-sm mt-2 flex-1">{product.description}</p>
