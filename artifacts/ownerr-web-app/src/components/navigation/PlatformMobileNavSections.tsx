...
import { NAV_ITEMS, getProductsDropdownSections } from '@/routes/publicNavConfig';
    ...
    <Collapsible className="group/navcoll rounded-[10px] border border-[color:var(--terminal-border)]/60 bg-[color:var(--terminal-bg)]/40">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-4 py-3.5 text-left text-[color:var(--terminal-fg)]">
        <span>
          ...
          <span className="mt-0.5 block text-[11px] text-[color:var(--terminal-muted)]">All products & apps</span>
        </span>
      ...
      <CollapsibleContent className="border-t border-[color:var(--terminal-border)]/50 px-2 py-2">
        {sections.map((section) => (
          <div key={section.id} className={cn(section.id !== 'hub' && 'mt-2 border-t border-[color:var(--terminal-border)]/40 pt-2')}>
            {groupLabel(section.title)}
            {section.items.map((item) => (
              <SheetClose key={item.id} asChild>
                <Link href={item.href} className={nestedLink(isNavLinkActive(location, item.href))} onClick={onNavigate}>
                  {item.label}
                </Link>
              </SheetClose>
            ))}
...
export function PlatformMobileNavSections({ location, onNavigate, linkClass }: Props) {
  return (
    <>
      <MobileProductsAccordion location={location} onNavigate={onNavigate} linkClass={linkClass} />
      {NAV_ITEMS.map((item) => (
        <SheetClose key={item.id} asChild>
          <Link href={item.href} className={linkClass(isNavLinkActive(location, item.href))} onClick={onNavigate}>
            {item.label}
          </Link>
        </SheetClose>
      ))}
