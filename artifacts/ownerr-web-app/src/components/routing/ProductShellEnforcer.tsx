...
import { isProductAuthPath, parseProductSlugFromAuthPath } from '@/lib/auth/productAuthRoutes';
import { productDashboardPath } from '@/lib/auth/productLock';
import { resolveMembershipAppForPath } from '@/lib/platform/appMembership';
import { normalizePathname, isProductAppInternalPath } from '@/routing/routeResolver';
...
function isOwnerrOsPublicBrowsePath(pathname: string): boolean {
  ...
  if (path === PRODUCT_ROUTES.ownerrOsLanding || path === PRODUCT_ROUTES.ownerrOsJoin) return true;
  if (path.startsWith('/share/founder/')) return true;
  if (path === PUBLIC_ROUTES.pricing || path === PUBLIC_ROUTES.contact) return true;
  return false;
}

function shouldLockOwnerrOsShell(pathname: string, activeProduct: string | null): boolean {
...
export function ProductShellEnforcer() {
  ...
  }, [loading, session, activeProduct, location, navigate, setActiveProduct]);
