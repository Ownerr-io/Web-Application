...
export function AcquireBidPanel({
  ...
  const minBid = useMemo(() => minNextBidAmount(base, highest), [base, highest]);
    ...
    >
      <div className="flex items-center justify-between gap-2 text-muted-foreground">
        <span className="inline-flex items-center gap-1">
          ...
          {highest != null ? `High bid ${formatShortCurrency(highest)}` : `From ${formatShortCurrency(minBid)}`}
        </span>
        {!currentUser ? (
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <Button type="submit" size={compact ? 'sm' : 'default'} disabled={createBid.isPending}>
          {createBid.isPending ? 'Submitting…' : 'Place bid'}
        </Button>
      </div>
      {!compact ? (
        <div>
          <Label htmlFor={`bid-msg-${startup.slug}`} className="text-xs text-muted-foreground">
            Message (optional)
          </Label>
          <Textarea
