...
export function ProductNav({
  ...
  const sheetControlled =
    typeof mobileSheetOpen === "boolean" &&
    typeof onMobileSheetOpenChange === "function";
    ...
    <div
      className={cn(
        ...
        enableScrollChrome &&
          scrolled &&
          !terminal &&
          "border-b border-border/80 bg-background/95 shadow-sm backdrop-blur-lg",
      )}
    >
      <nav
        className={cn(
          NAV_SHELL,
          "flex flex-nowrap items-center justify-between gap-3 py-2.5 sm:py-3",
        )}
        aria-label="Primary"
      >
        ...
        <div className="shrink-0 lg:hidden">
          ...
          >
            ...
            >
              ...
              <nav
                className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto py-4 pr-2"
                aria-label="Mobile"
              >
                <PlatformMobileNavSections
                  location={location}
                  onNavigate={onNavigate}
                  linkClass={linkMobileClass}
                />
              </nav>
      ...
      {showProductContext && productContext ? (
        <ProductContextBar terminal={terminal} />
      ) : null}
    </div>
  );
}
