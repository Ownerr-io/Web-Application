...
export function OwnerrOsEditStartupForm({ record, mode = record ? 'edit' : 'create', onSaved, onCancel }: Props) {
  ...
  async function onSave() {
    if (fullName.trim().length < 2 || startupName.trim().length < 2 || ideaDescription.trim().length < 12) {
    ...
    try {
      ...
      const { record: updated } = await updateFounderSubmission(record.id, input);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4 rounded-xl border border-border bg-muted/20 p-6">
        <h2 className="text-lg font-bold">{mode === 'create' ? 'Create your startup listing' : 'Edit startup'}</h2>
        ...
        <div className="space-y-2">
          ...
          <Input id="edit-startup" value={startupName} onChange={(e) => setStartupName(e.target.value)} />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Textarea id="edit-desc" rows={4} value={ideaDescription} onChange={(e) => setIdeaDescription(e.target.value)} />
        </div>
        <Button type="button" className="btn-platform-gradient w-full font-bold" disabled={saving} onClick={() => void onSave()}>
