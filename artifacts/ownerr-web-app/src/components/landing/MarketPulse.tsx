...
function Strip({ ariaHidden }: { ariaHidden?: boolean }) {
  return (
    <div className="flex shrink-0 items-center gap-2 px-1" aria-hidden={ariaHidden}>
      {ITEMS.map((it, i) => (
        <div
          key={`${it.label}-${i}`}
        ...
        >
          <span className="text-[10px] font-bold uppercase tracking-wider text-[color:var(--terminal-muted)]">{it.label}</span>
          <span className="text-[11px] font-bold tabular-nums text-[color:var(--terminal-ochre)]">{it.value}</span>
        </div>
      ))}
    </div>
  );
