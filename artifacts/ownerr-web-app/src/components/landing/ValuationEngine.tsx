...
export function ValuationEngine({ value, onChange }: Props) {
  ...
  const [internal, setInternal] = useState<ValuationInputs>(DEFAULT_VALUATION_INPUTS);
  ...
  return (
    <div className={cnShell()}>
      <div className={`border-b border-[color:var(--terminal-border)] px-4 py-3 ${pad}`}>
        <h2 className="text-sm font-bold uppercase tracking-[0.18em] text-[color:var(--terminal-muted)]">Valuation engine</h2>
        <p className="mt-1 text-xs text-[color:var(--terminal-muted)]">
          Inputs stream through a private scenario model — outputs are illustrative, not a fairness opinion.
        </p>
      </div>
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className={`space-y-4 border-[color:var(--terminal-border)] lg:border-r ${pad}`}>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="MRR ($)">
              <Input
                ...
                value={i.mrr || ''}
                onChange={(e) => update((s) => ({ ...s, mrr: num(e.target.value, s.mrr) }))}
              />
            </Field>
            <Field label="ARR ($)">
              <Input
                ...
                className="font-mono text-xs"
                value={i.arr || ''}
                onChange={(e) => update((s) => ({ ...s, arr: num(e.target.value, 0) }))}
              />
            </Field>
            <Field label="Profit margin %">
              <Input
                ...
                onChange={(e) => update((s) => ({ ...s, profitMarginPct: num(e.target.value, s.profitMarginPct) }))}
              />
            </Field>
            <Field label="Operating expenses / mo ($)">
              <Input
                ...
                onChange={(e) =>
                  update((s) => ({ ...s, operatingExpensesMonthly: num(e.target.value, s.operatingExpensesMonthly) }))
                }
              />
            </Field>
            <Field label="Monthly growth %">
              <Input
                ...
                onChange={(e) => update((s) => ({ ...s, monthlyGrowthPct: num(e.target.value, s.monthlyGrowthPct) }))}
              />
            </Field>
            <Field label="Churn % (monthly)">
              <Input
                ...
                onChange={(e) => update((s) => ({ ...s, churnPctMonthly: num(e.target.value, s.churnPctMonthly) }))}
              />
            </Field>
            <Field label="CAC ($)">
              <Input
                ...
                className="font-mono text-xs"
                value={i.cac}
                onChange={(e) => update((s) => ({ ...s, cac: num(e.target.value, s.cac) }))}
              />
            </Field>
            <Field label="LTV ($)">
              <Input
                ...
                className="font-mono text-xs"
                value={i.ltv}
                onChange={(e) => update((s) => ({ ...s, ltv: num(e.target.value, s.ltv) }))}
              />
            </Field>
            <Field label="Burn multiple">
              <Input
                ...
                onChange={(e) => update((s) => ({ ...s, burnMultiple: num(e.target.value, s.burnMultiple) }))}
              />
            </Field>
            <Field label="Runway (months)">
              <Input
                ...
                onChange={(e) => update((s) => ({ ...s, runwayMonths: num(e.target.value, s.runwayMonths) }))}
              />
            </Field>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="Industry">
              <Select value={i.industry} onValueChange={(v) => update((s) => ({ ...s, industry: v }))}>
            ...
            <Field label="Stage">
              <Select value={i.stage} onValueChange={(v) => update((s) => ({ ...s, stage: v }))}>
                <SelectTrigger className="font-mono text-xs">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <Field label="Geography">
              <Select value={i.geography} onValueChange={(v) => update((s) => ({ ...s, geography: v }))}>
                <SelectTrigger className="font-mono text-xs">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <Field label="Founder goal">
              <Select
                value={i.founderGoal}
                onValueChange={(v) => update((s) => ({ ...s, founderGoal: v as FounderGoal }))}
              >
                <SelectTrigger className="font-mono text-xs">
                  <SelectValue />
                </SelectTrigger>
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-[10px] font-bold uppercase tracking-wider text-[color:var(--terminal-muted)]">Output stream</div>
            {!reduce ? (
              <span className="inline-flex items-center gap-1.5 rounded border border-dashed border-[color:var(--terminal-border)] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-[color:var(--terminal-muted)] tabular-nums">
                <motion.span
                  ...
                  animate={recalcPulse > 0 ? { scale: [1, 1.45, 1], opacity: [1, 0.75, 1] } : { scale: 1, opacity: 1 }}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Out
              ...
              emphasizeValues
            />
            <Out label="Investor interest" value={`${out.investorInterest} / 100`} reduce={reduce} emphasizeValues />
            <Out label="Acquisition heat" value={`${out.acquisitionHeat} / 100`} reduce={reduce} emphasizeValues />
            <Out label="Market timing" value={out.marketTiming} wide reduce={reduce} emphasizeValues />
            <Out label="Growth quality" value={`${out.growthQuality} / 100`} reduce={reduce} emphasizeValues />
            <Out label="Revenue stability" value={`${out.revenueStability} / 100`} reduce={reduce} emphasizeValues />
            <Out label="Liquidity signal" value={out.liquiditySignal} wide reduce={reduce} emphasizeValues />
            <Out label="Confidence %" value={`${out.confidencePct}%`} reduce={reduce} emphasizeValues />
            <Out label="Suggested timeline" value={out.suggestedTimeline} wide reduce={reduce} emphasizeValues />
          </div>
          <div className="rounded-[8px] border border-[color:var(--terminal-border)] bg-[color:var(--terminal-surface)] p-3 text-xs leading-snug text-[color:var(--terminal-muted)]">
            <span className="font-bold text-[color:var(--terminal-ochre)]">REC · </span>
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="space-y-1.5">
      <Label className="text-[10px] font-bold uppercase tracking-wider text-[color:var(--terminal-muted)]">{label}</Label>
      {children}
    </div>
  );
}
...
function Out({
  ...
  const shell = 'border-[color:var(--terminal-border)] bg-[color:var(--terminal-bg)]';
  const valueCls = accent ? 'text-[color:var(--terminal-ochre)]' : 'text-[color:var(--terminal-fg)]/90';
  ...
  const valueNode =
    emphasizeValues && !reduce ? (
      <motion.div
        ...
        className={cn('mt-1 font-mono text-sm font-bold tabular-nums', valueCls)}
      >
        {value}
      </motion.div>
    ) : (
      <div className={cn('mt-1 font-mono text-sm font-bold tabular-nums', valueCls)}>{value}</div>
    );

  return (
    <div className={cn(wide ? 'sm:col-span-2' : '', 'rounded-[8px] border px-3 py-2', shell)}>
      <div className="text-[10px] font-bold uppercase tracking-wider text-[color:var(--terminal-muted)]">{label}</div>
      {valueNode}
    </div>
  );
}
