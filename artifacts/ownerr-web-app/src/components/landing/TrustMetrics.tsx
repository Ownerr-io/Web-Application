    ...
    <div className="grid gap-4 lg:grid-cols-[1.1fr_minmax(0,0.9fr)] lg:items-start">
      <div>
        ...
        <h2 className="mt-2 text-2xl font-bold tracking-tight text-[color:var(--terminal-fg)] sm:text-3xl">
          Evidence-first listings
        </h2>
        <p className="mt-3 max-w-xl text-sm leading-relaxed text-[color:var(--terminal-muted)]">
          Verification gates reduce adverse selection. Signals compound into a
          readiness curve founders can improve before going live — buyers see
          the same progression in diligence view.
        </p>
      </div>
        ...
        {ROWS.map((r, i) => (
          <div key={r.label} className="space-y-1.5">
            <div className="flex items-center justify-between gap-2 text-xs font-bold text-[color:var(--terminal-fg)]">
              <span>{r.label}</span>
              <span className="tabular-nums text-[color:var(--terminal-ochre)]">
                {r.score}%
              </span>
            </div>
