  ...
  return (
    <div
      className={cn(
        ...
        terminalPalette
          ? "landing-terminal-palette bg-[color:var(--terminal-bg)] selection:bg-[color:var(--terminal-ochre)] selection:text-[#0b0b0c]"
    ...
    >
      {terminalPalette ? (
        <header className="sticky top-0 z-50 w-full border-b border-transparent bg-transparent pt-[env(safe-area-inset-top,0px)] text-[color:var(--terminal-fg)]">
          <ProductNav
            terminalChrome
            enableScrollChrome
            showProductContext={!hideProductContext}
          />
        </header>
      ) : (
      ...
      <main
        ...
        className={cn(
          ...
          hideFooter ? "pb-[env(safe-area-inset-bottom,0px)]" : undefined,
      ...
      {!hideFooter ? (
        <PlatformFooter
          ...
          compactBottom
          className="mt-auto shrink-0 border-t-0"
        />
      ) : null}
    </div>
  );
