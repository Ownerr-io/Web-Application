...
import { parseProductSlugFromAuthPath, resolveProductAuthPath } from '@/lib/auth/productAuthRoutes';
import {
...
export function ProductAuthCallbackPage() {
  ...
  const appSlug = parseProductSlugFromAuthPath(location.split('?')[0] ?? location);

  useEffect(() => {
    if (loading) return;
    if (!appSlug || !isAppSlug(appSlug)) {
      logProductIssue('auth.callback', new Error('invalid product in callback path'));
