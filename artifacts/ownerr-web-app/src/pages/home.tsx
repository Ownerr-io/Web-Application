...
export default function Home() {
  ...
  const leaderboardRows = useMemo(() => {
    return [...mergedStartups].sort(
      (a, b) =>
        leaderboardMetricValue(b, metric, period) -
        leaderboardMetricValue(a, metric, period),
    );
  ...
  const bestDeals = publicStartups
    .filter((s) => s.forSale && s.multiple && s.multiple < 1.8)
    .slice(0, 8);
    ...
    <div className="flex flex-col gap-4">
      ...
      <section className="min-w-0">
        ...
        <div className="flex w-full min-w-0 overflow-x-auto pb-1 gap-3 snap-x snap-mandatory hide-scrollbar max-lg:-mx-4 max-lg:px-4">
          {recentlyListed.map((s) => (
            <div
              key={s.slug}
              className="min-w-[210px] w-[210px] snap-start shrink-0"
            >
          ...
          <div className="flex gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40">
                <DropdownMenuRadioGroup
                  value={metric}
                  onValueChange={(v) => setMetric(v as "mrr" | "arr")}
                >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  className="text-xs px-3 py-1.5 bg-card border border-border rounded-[8px] flex items-center gap-1 font-bold hover:bg-muted transition-colors"
                >
                  {period === "all_time" ? "All time" : "Current"}{" "}
                  <ChevronDown className="w-3 h-3" />
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-44">
                <DropdownMenuRadioGroup
                  value={period}
                  onValueChange={(v) => setPeriod(v as "all_time" | "current")}
                >
              ...
              <tbody>
                {leaderboardRows.slice(0, 25).map((startup, index) => {
                  const founder = founders.find(
                    (f) => f.handle === startup.founderHandle,
                  );
                  const metricVal = leaderboardMetricValue(
                    startup,
                    metric,
                    period,
                  );
                  ...
                  return (
                    ...
                    >
                      <td className="px-2 py-1.5 text-center font-bold text-sm">
                        {rankDisplay}
                      </td>
                      <td className="px-2 py-1.5">
                        <Link
                          href={`${marketplacePath(`/startup/${startup.slug}`)}?from=leaderboard`}
                        >
                          <div className="flex items-center gap-2 group cursor-pointer">
                            ...
                            <div className="min-w-0">
                              <div className="font-bold text-sm group-hover:underline flex items-center gap-1.5 flex-wrap">
                                ...
                                {startup.forSale && (
                                  <span className="mp-badge-for-sale text-[8px] px-1 py-0.5 rounded font-bold uppercase tracking-wider whitespace-nowrap">
                                    FOR SALE
                                  </span>
                                )}
                              </div>
                              <div className="text-[10px] text-muted-foreground max-w-[240px] line-clamp-1">
                                {startup.description}
                              </div>
                            </div>
                          </div>
                      ...
                      <td className="px-2 py-1.5">
                        {founder ? (
                          <Link
                            href={marketplacePath(`/founder/${founder.handle}`)}
                          >
                          ...
                          <Link
                            href={marketplacePath(
                              `/founder/${encodeURIComponent(startup.founderHandle)}`,
                            )}
                          >
                      ...
                      <td className="px-2 py-1.5 text-right font-bold text-sm tabular-nums">
                        ...
                        >
                          {startup.momGrowth > 0
                            ...
                            : startup.momGrowth < 0
                              ...
                              : "—"}
                          {startup.momGrowth !== 0 &&
                            ` ${Math.abs(startup.momGrowth).toFixed(0)}%`}
                        </div>
                      </td>
                    </motion.tr>
                  );
      ...
      <section className="min-w-0">
        ...
        <div className="grid grid-cols-1 gap-3 min-[400px]:grid-cols-2 lg:grid-cols-4 lg:gap-4">
          <KPI
            label="Total Verified MRR"
            value={formatCurrency(
              mergedStartups.reduce((s, x) => s + x.revenue, 0),
            )}
          />
          <KPI
            label="Verified Startups"
            value={String(publicStartups.length)}
          />
          <KPI label="Founders" value={String(founders.length)} />
          <KPI
            label="Listings for sale"
            value={String(publicStartups.filter((s) => s.forSale).length)}
            accent
          />
        </div>
      </section>
  ...
  return (
    <div
      className={`startup-card flex min-w-0 flex-col items-center justify-center p-4 text-center sm:p-5 ${
        accent
          ? "border border-[color:var(--terminal-ochre-muted)] bg-[color:var(--terminal-surface-2)]"
          : ""
      }`}
    >
