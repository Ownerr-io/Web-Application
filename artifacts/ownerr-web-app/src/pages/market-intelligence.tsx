...
function toneClass(tone: (typeof PANELS)[number]["tone"]) {
  if (tone === "up" || tone === "hot")
    return "text-[color:var(--terminal-lime)]";
    ...
    <MarketingLayout>
      <div
        className={cn(
          ...
          "space-y-10 py-10 sm:space-y-12 sm:py-14",
        )}
      >
        <header className="space-y-3.5 border-b border-[color:var(--terminal-border)] pb-8">
          ...
          <p className="mt-2 max-w-2xl text-sm font-medium leading-relaxed text-[color:var(--terminal-muted)]">
            Rolling desk notes distilled into a terminal-native snapshot.
            Figures are illustrative composites — pair with your own scenario
            outputs before allocation decisions.
          </p>
        </header>
        ...
        <section className="grid gap-3 sm:grid-cols-3">
          {PULSE.map((p) => (
            ...
            >
              ...
              <p className="mt-1 font-mono text-xl font-bold tabular-nums text-[color:var(--terminal-ochre)]">
                {p.value}
              </p>
        ...
        <section className="overflow-hidden rounded-[10px] border border-[color:var(--terminal-border)] bg-[color:var(--terminal-surface)]">
          <div className="border-b border-[color:var(--terminal-border)] px-4 py-3">
            <h2 className="text-balance text-base font-bold">
              Sector multiple monitor
            </h2>
            <p className="mt-1 text-xs text-[color:var(--terminal-muted)]">
              Indicative bands · not investment advice
            </p>
          </div>
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              ...
              <tbody>
                {SECTOR_ROWS.map((row) => (
                  <tr
                    key={row.sector}
                    className="border-b border-[color:var(--terminal-border)]/60 last:border-0"
                  >
                    <td className="px-4 py-3 font-semibold text-[color:var(--terminal-fg)]">
                      {row.sector}
                    </td>
                    <td className="px-4 py-3 font-mono tabular-nums text-[color:var(--terminal-ochre)]">
                      {row.multiple}
                    </td>
                    <td className="px-4 py-3 text-[color:var(--terminal-muted)]">
                      {row.trend}
                    </td>
                    <td className="px-4 py-3 text-xs leading-relaxed text-[color:var(--terminal-muted)]">
                      {row.note}
                    </td>
                  </tr>
                ))}
            ...
            <div className="flex w-full flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-3 max-w-none text-sm leading-relaxed text-[color:var(--terminal-muted)]">
                  Buyers are underwriting slower but paying for verified
                  retention and margin durability. Listings with a full
                  verification stack clear diligence in fewer cycles — route
                  through Ownerr verification before broad outreach.
                </p>
                <p className="mt-3 max-w-none text-sm leading-relaxed text-[color:var(--terminal-muted)]/90">
                  Next refresh: composite roll-forward after weekly strategic
                  bid tape. Export scenario PDFs from valuation before sharing
                  externally.
                </p>
              </div>
            </div>
