  ...
  if (records.length === 0) {
    return (
      <OwnerrOsAppPageShell
        title="Analytics"
        description="Complete a founder listing to unlock visit and referral metrics."
      >
        <Link
    ...
    >
      <dl className="grid w-full grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-border bg-muted/20 p-5">
          <dt className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Startups</dt>
          <dd className="mt-2 text-3xl font-bold tabular-nums text-foreground">{records.length}</dd>
        </div>
        <div className="rounded-xl border border-border bg-muted/20 p-5">
          <dt className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Profile visits</dt>
          <dd className="mt-2 text-3xl font-bold tabular-nums text-foreground">{totals.visits}</dd>
        </div>
        <div className="rounded-xl border border-border bg-muted/20 p-5">
          <dt className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Referral signups</dt>
          <dd className="mt-2 text-3xl font-bold tabular-nums text-foreground">{totals.signups}</dd>
        </div>
      </dl>

      <div className="w-full space-y-3">
        <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">By startup</h2>
              ...
              {records.map((row) => (
                <TableRow key={row.id} className="cursor-pointer hover:bg-muted/30">
                  <TableCell className="font-semibold">
                    <Link href={ownerrOsListingDetailPath(row.id)} className="hover:text-primary hover:underline">
                      {row.startupName}
                    </Link>
                  </TableCell>
      ...
      <p className="text-xs text-muted-foreground">
        Share links and track referrals from{' '}
        <Link href={PRODUCT_ROUTES.ownerrOsReferrals} className="font-bold text-primary hover:underline">
          Invite &amp; Stats
        </Link>
        .
      </p>
