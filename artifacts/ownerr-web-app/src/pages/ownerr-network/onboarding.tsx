...
const STEPS: Step[] = [
  { kind: "name" },
  { kind: "username" },
  ...OWNERR_NETWORK_ONBOARDING_STEPS.map((_, index) => ({
    kind: "mcq" as const,
    index,
  })),
];
...
function OnboardingExperience() {
  ...
  function advanceMcq(value: string) {
    const q =
      OWNERR_NETWORK_ONBOARDING_STEPS[step.kind === "mcq" ? step.index : 0];
    if (!q) return;
  ...
  return (
    <ValuationFullViewport className="grid grid-rows-[auto_1fr_auto] py-4">
      ...
      <div
        className={cn(MARKETING_SHELL_CLASS, "relative z-20 px-4 pt-3 sm:pt-6")}
      >
      ...
      <div
        className={cn(
          ...
          "relative z-20 flex min-h-0 items-center justify-center overflow-y-auto overscroll-y-contain px-4 py-6 sm:py-8",
        )}
      >
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {step.kind === "name" ? (
              ...
              >
                <h2 className="text-3xl font-bold text-[color:var(--terminal-display)] tracking-tight">
                  Your name
                </h2>
                ...
                <div className="mt-10 sm:mt-12">
                  <input
                    ...
                    className={cn(
                      ...
                      error &&
                        "border-b-[color:var(--terminal-ochre)] focus:border-[color:var(--terminal-ochre)]",
                    )}
                  />
                </div>
              </form>
            ...
            {step.kind === "username" ? (
              ...
              >
                ...
                <p className="mt-3.5 text-sm font-semibold text-[color:var(--terminal-muted)] tracking-wide">
                  Choose a unique handle for your profile directory. (Min 3
                  characters)
                </p>
                <div className="mt-10 sm:mt-12">
                  <input
            ...
            {step.kind === "mcq" ? (
              <div className="w-full z-10">
                ...
                <ul className="mt-10 sm:mt-12 space-y-2.5">
                  {OWNERR_NETWORK_ONBOARDING_STEPS[step.index]?.options.map(
                    (opt) => (
                      <li key={opt}>
                        <Button
                          type="button"
      ...
      <div
        className={cn(
          "z-30 shrink-0 bg-transparent pt-3 sm:relative sm:pt-6 pb-[max(0.75rem,env(safe-area-inset-bottom,0px))] sm:pb-[max(1.5rem,env(safe-area-inset-bottom,0px))]",
          MARKETING_SHELL_CLASS,
        )}
      >
